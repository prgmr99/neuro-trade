...
async function runTest() {
  ...
  try {
    ...
    // Client timer loop simulating useRoomBattle
    let clientAdvancementAttempts = 0;
    const clientTimer = setInterval(() => {
        ...
        // This mirrors the code in useRoomBattle.ts
        if (remaining <= 0 && client.roomState.host_id === client.userId) {
            clientAdvancementAttempts++;
        } else if (remaining <= 0) {
            // Client would want to advance, but code prevents it
            console.log("[Client] Timer reached 0, but I am not the host!");
        }
    }, 1000);
    ...
    if (client.roomState.day === 1) {
        console.log("SUCCESS: Vulnerability confirmed! The day did not advance because the host is offline, and the client is restricted from advancing it.");
    } else {
        throw new Error("Test failed: Day advanced somehow.");
    }
