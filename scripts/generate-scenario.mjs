 ...
 * @param {unknown} arc
 * @returns {string[]} list of violation strings (empty = valid)
 */
function validateArc(arc) {
  const violations = [];

  if (!arc || typeof arc !== 'object') {
    return ['Arc is not an object'];
  }

  if (!Array.isArray(arc.news)) {
    return ['arc.news is missing or not an array'];
  }
  ...
  for (let i = 0; i < arc.news.length; i++) {
    const item = arc.news[i];
    const prefix = `news[${i}] (id=${item?.id ?? '?'})`;
    ...
    // dayIdx
    const day = item.dayIdx;
    if (![1, 2, 3, 4, 5].includes(day)) {
      violations.push(`${prefix}: dayIdx ${day} is not in 1..5`);
    } else {
      byDay[day]++;
    }
    ...
    if (!item.id || typeof item.id !== 'string') {
      ...
      violations.push(`${prefix}: duplicate id "${item.id}"`);
    } else {
      seenIds.add(item.id);
    }
    ...
    if (!item.title?.en || !item.title?.ko) {
      violations.push(`${prefix}: title missing en or ko`);
    }

    // bilingual content
    if (!item.content?.en || !item.content?.ko) {
      violations.push(`${prefix}: content missing en or ko`);
    }

    // effect
    if (!item.effect || typeof item.effect !== 'object') {
      violations.push(`${prefix}: effect is missing or not an object`);
    } else {
      const entries = Object.entries(item.effect);
      if (entries.length === 0) {
      ...
      for (const [ticker, mult] of entries) {
        ...
        if (typeof mult !== 'number' || mult < 0.85 || mult > 1.15) {
          violations.push(
            `${prefix}: effect[${ticker}]=${mult} is outside [0.85, 1.15]`
          );
        }
      }
    }
  ...
  // Total count
  if (arc.news.length !== 25) {
    violations.push(`Total news items = ${arc.news.length}, expected 25`);
  }

  // Per-day count
  for (const d of [1, 2, 3, 4, 5]) {
    if (byDay[d] !== 5) {
      violations.push(`dayIdx ${d} has ${byDay[d]} news items, expected 5`);
    }
  }

  // Ticker diversity
  if (seenTickers.size < 3) {
    violations.push(
      `Only ${seenTickers.size} tickers appear in effects (need at least 3). ` +
      `Seen: ${[...seenTickers].join(', ')}`
    );
  }
